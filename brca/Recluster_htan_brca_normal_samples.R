### subset and recluster cells from normal samples
...
panc.my <- subset(x = panc, cells = rownames(subset(panc@meta.data, tissue_type == 'normal')))
cat('SCTransform\n')
...
for (ct in c('mCAF', 'Normal_duct', 'Endothelial')) {
  cat('Subset not tumor cells cells\n')
  panc.my.ct <- subset(x = panc.my, cells = rownames(subset(panc.my@meta.data, cell_type_specific == ct)))
  cat('SCTransform\n')
