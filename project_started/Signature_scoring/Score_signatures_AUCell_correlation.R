...
Run_AUCell <- function(obj, geneset.list, name) {
    ...
    print(DimPlot(obj, group.by = 'cell_type_merged4', cols = 'Dark2', label = TRUE))
...
msig_df <- c2_gene_sets %>% dplyr::filter(grepl('SENESCE|senesce|Senesce|SASP|TP53|RB1', gs_name)) %>% 
...
msig_df <- c5_gene_sets %>% dplyr::filter(grepl('SENESCE|senesce|Senesce|SASP|TP53|RB1', gs_name)) %>% 
  dplyr::distinct(gs_name, gene_symbol) %>% 
  as.data.frame()
