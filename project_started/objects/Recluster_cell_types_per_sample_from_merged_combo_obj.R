# Recluster cell types in each single sample
###libraries
##################
library(future)
...
samples.in.object <- unique(panc.my$Sample)
print(samples.in.object)
print(dim(panc.my))


cell.types.touse %>% walk (function(ct) {
  print(ct)
  samples.in.object %>% walk (function(s) {
    print(s)
    int.sub <- subset(x = panc.my, 
                      cells = rownames(dplyr::filter(panc.my@meta.data, .data[[cell_column]] == ct & Sample == s)))
    print(dim(int.sub))
    ...
    if (ncol(int.sub) > 300) {
      ...
      saveRDS(int.sub,  paste0(s,"_combo_",ct, ".rds"))
    }
