# add All tumor cells to the mCRC object for Xenium label transfer
...
if(!file.exists(paste0(add_filename,".rds"))) {
  ...
  tumor.panc.tumor <- subset(tumor.panc, cells = cells.toadd)
  tumor.panc.tumor@meta.data[[cell_column]] <- 'Tumor'
  
  common.columns <- intersect(colnames(tumor.panc.tumor@meta.data), colnames(panc.my@meta.data))
  tumor.panc.tumor@meta.data <- tumor.panc.tumor@meta.data[,common.columns]
  
  
  int.sub <- merge(panc.my, tumor.panc.tumor)
...
int.sub@meta.data <- int.sub@meta.data %>% mutate(cell_type_broad = case_when(grepl('Hepatocytes', cell_type_sen_mCRC) ~ 'Hepatocytes',
                                     grepl('fibroblas', cell_type_sen_mCRC) ~ 'Portal fibroblasts',
                                     grepl('HSC', cell_type_sen_mCRC) ~ 'Hepatic stellate cells',
                                     grepl('PS Mid lobular LSEC', cell_type_sen_mCRC) ~ 'Mid lobular LSECs',
