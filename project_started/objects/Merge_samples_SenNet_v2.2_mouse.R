...
if (dt.tofilter == 'Combo RNA') {
  if (!file.exists(paste0(length(samples.id),"_Mouse_Merged_not_normalized_",add_filename,".rds"))) {
    ...
    combined <- readRDS(paste0(length(samples.id),"_Mouse_Merged_not_normalized_",add_filename,".rds"))
  }
  ...
  ggsave(paste0(length(samples.id),"_Mouse_Merged_clusters_", add_filename, ".pdf"),height=10,width=30)
  ...
  p1 + p2 + p3 & NoLegend() & theme(plot.title = element_text(hjust = 0.5))
  ggsave(paste0(length(samples.id),"_Mouse_Merged_Sample_", add_filename, ".pdf"),height=10,width=30)
  ...
  p1 + p2 + p3 & NoLegend() & theme(plot.title = element_text(hjust = 0.5))
  ggsave(paste0(length(samples.id),"_Mouse_Merged_Age.group_", add_filename, ".pdf"),height=10,width=30)
  ...
  p1 + p2 + p3 & NoLegend() & theme(plot.title = element_text(hjust = 0.5))
  ggsave(paste0(length(samples.id),"_Mouse_Merged_cell_type_", add_filename, ".pdf"),height=10,width=30)
  ...
  ggsave(paste0(length(samples.id),"_Mouse_Merged_Sample_", add_filename, ".pdf"),height=10,width=12)
  DimPlot(combined, reduction = "umap.rna", group.by = "Age.group", label = TRUE, label.size = 2.5, repel = TRUE)
  ggsave(paste0(length(samples.id),"_Mouse_Merged_Age.group_", add_filename, ".pdf"),height=10,width=11)
  
  
  DimPlot(combined, reduction = "umap.rna", group.by = "cell_type", label = TRUE, label.size = 2.5, repel = TRUE)
  ggsave(paste0(length(samples.id),"_Mouse_Merged_cell_type_", add_filename, ".pdf"),height=10,width=11)
