...
wd <- '~/lab_Ding/work/single_cell/senescence/ccrcc/normal_samples'
...
print(DimPlot(panc, group.by = 'Cell_type.detailed', label = T)& coord_fixed())
print(DimPlot(panc, group.by = 'Easy_id', label = T)& coord_fixed())
dev.off()
...
dir.create(fd, showWarnings = F, recursive = T)
for(s in signatures$ont %>% unique) {
  do.cor.good.cover (panc, s, ct = cell.type, dot.size = 0.5, cell.pct.cutoff = 0.05, cor.adjust.method = 'none', folder = fd)
}

fd <- 'no_fdr_coverage1pct/'
dir.create(fd, showWarnings = F, recursive = T)
for(s in signatures$ont %>% unique) {
  do.cor.good.cover (panc, s, ct = cell.type, dot.size = 0.5, cell.pct.cutoff = 0.01, cor.adjust.method = 'none', folder = fd)
}
...
print(FeaturePlot(panc, features = c('MKI67', 'TOP2A', 'MYBL2', 'BUB1', 'PLK1'), order = T ,ncol = 4)&coord_fixed())
print(FeaturePlot(panc, features = c('H2AFX','H2AFY', 'TP53BP1'), order = T, ncol = 4)&coord_fixed())
dev.off()

panc$seurat_clusters
getwd()
pdf(paste0('VLNplots_',cell.type,'.pdf'), width = 12, height =12, useDingbats = F)
DimPlot(panc, group.by = 'seurat_clusters', label = T)&coord_fixed()
VlnPlot(object = panc, features = c('CDKN1A','CDKN2A', 'TP53', 'CCND1'), pt.size = 0.1, group.by = 'seurat_clusters', ncol = 2)
VlnPlot(object = panc, features = c('MKI67', 'TOP2A', 'MYBL2', 'BUB1', 'PLK1'), pt.size = 0.9, group.by = 'seurat_clusters', ncol = 2)
VlnPlot(object = panc, features = c('H2AFX','H2AFY', 'TP53BP1'), pt.size = 0.9, group.by = 'seurat_clusters', ncol = 2)
VlnPlot(object = panc, features = c('SERPINE1','LMNB1'), pt.size = 0.9, group.by = 'seurat_clusters', ncol = 2)
VlnPlot(object = panc, features = c('IL1A', 'IL6', 'CXCL8', 'TNF', 'CCL2', 'MMP1', 'MMP3', 'IGFBP7'), pt.size = 0.9, group.by = 'seurat_clusters', ncol = 2)
dev.off()
