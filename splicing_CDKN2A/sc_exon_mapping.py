...
#Usage: sc_exon_mapping.py -b sample.bam -c coordinates -r barcodes
...
for k in position:
    ...
    junction_key=chr+"__"+start+"__"+stop
    ...
    reads=process.split("\n")
    if len(reads)==1:
      print('no reads for '+search_junction)
      continue
    junction_count=0
    total_reads=0
    for read in reads:
    	...
    	if not re.match("\tCB:",read): #only keep reads that have cell barcode details
    		...
    		if re.match("150M",read_details[5]):

    			for element in read_details:
    				if re.match("^CB:",element):
    					cellbarcode=element.split(":")[2]
    					junction_bc[junction_key].append(cellbarcode)
    					isoformtype=position[junction_key]
...
outfile=sampleinfo+"_exons_bc.txt"
fh = open(outfile, "w")
