...
do.cor.good.cover <- function(panc, s, ct, dot.size = 1, cell.pct.cutoff = 0.05, cor.adjust.method = 'fdr', folder ) {
  ...
  ggplot(data = toplot,  
    ...
    ggtitle (paste0('Correlation with\n', s)) +
    scale_color_gradient2(name = "Spearman's rho", low = '#1f78b4', mid = 'lightgrey', high = '#e41a1c', limits = c(-max.cor, max.cor))
  ggsave(paste0(folder, 'Featureplot_', ct, '_cor_', s, '_splitted.pdf'), useDingbats = F, width = 15, height = 5.5)
...
wd <- '~/lab_Ding/work/single_cell/senescence/mouse_BM/senescence_correlation/Fibro/'
dir.create(wd, recursive = T)
setwd(wd)
panc$orig.ident
DimPlot(panc,label = T, cells = rownames(subset(panc@meta.data, orig.ident != 'Second_DOXO')))
ggsave('Dimplot_cluster.pdf', width = 5, height = 5, useDingbats = F)

DimPlot(panc, group.by ='Cell_type',  label = T, cells = rownames(subset(panc@meta.data, orig.ident != 'Second_DOXO')))
ggsave('Dimplot_cell_type_no_DOXO.pdf', width = 6, height = 5, useDingbats = F)

DimPlot(panc, group.by ='orig.ident',  label = T, cols =c( '#fc8d62','#8da0cb', '#66c2a5'))
ggsave('Dimplot_sample.pdf', width = 6, height = 5, useDingbats = F)
...
#do.cor.good.cover (panc = panc, s = 'Universal_sen', ct = cell.type, dot.size = 0.1, cell.pct.cutoff = 0.05,cor.adjust.method = 'fdr', folder = fd)
...
FeaturePlot(panc, markers.from.email$Generic, order = T)
FeaturePlot(panc, markers.from.email$Cell_cycle, order = T)
FeaturePlot(panc, markers.from.email$DDR, order = T)
FeaturePlot(panc, markers.from.email$SASP, order = T)
