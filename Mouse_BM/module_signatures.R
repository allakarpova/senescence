...
panc <- readRDS('~/lab_Ding/work/single_cell/senescence/objects/Cell_type.object.')
...
wd <- '~/lab_Ding/work/single_cell/senescence/mouse_BM/senescence_modules_scores_mouse_sc_paper/'
...
for(s in names(total.sen)) {
 # s <- 'Sen.core'
  ...
  VlnPlot(object = panc, features = s, pt.size = 0.9, group.by = 'seurat_clusters', ncol = 1, split.by = 'orig.ident',
          assay = 'RNA',cols =c( '#fc8d62','#8da0cb', '#66c2a5'))
  ...
  FeaturePlot(panc, features = s, order = T, min.cutoff = 0,split.by = 'orig.ident')
  ggsave(paste0('Featureplot_',cell.type, '_scores_', s, '_splitted.pdf'), useDingbats = F, width = 18, height = 5.8)
  
  FeaturePlot(panc, features = total.sen[[s]], order = T, min.cutoff = 0,split.by = 'orig.ident')
  ...
  ggplot (panc@meta.data, aes_string (x = s, fill = 'orig.ident')) +
