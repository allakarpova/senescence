...
wd <- '~/lab_Ding/work/single_cell/senescence/pdac/normal_samples'
dir.create(wd, showWarnings = F, recursive = T)
setwd(wd)

panc <- readRDS('~/lab_Ding/work/single_cell/senescence/objects/NAT_senescence_annotated_CAF_v3.rds')
...
print(DimPlot(panc, group.by = 'case', label = T)& coord_fixed())
dev.off()
...
panc$SCT_snn_res.2
getwd()
pdf(paste0('VLNplot_',cell.type,'.pdf'), width = 12, height =6, useDingbats = F)
